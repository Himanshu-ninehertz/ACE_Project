...
def measure_ducts(pdf_path):
    ...
    results = {
        ...
        "supply": {"lengths_mm": {}, "counts": {}},
        "extract": {"lengths_mm": {}, "counts": {}},
    }
...
def export_results_to_excel(results, excel_path, pdf_path):
...
def generate_pdf_report(results, output_path, pdf_path):
    """Generate a PDF report with measurement results for both supply (red) and extract (blue) ductwork"""
    ...
    story.append(Paragraph("""
        This report contains measurements for two types of ductwork:
        • Supply Ductwork (shown in RED on the drawing)
        • Extract Ductwork (shown in BLUE on the drawing)
    """, description_style))
    
    # Add scale information
    if results.get("scale_denominator"):
        story.append(Paragraph(f"Scale: 1:{int(results['scale_denominator'])}", description_style))
        story.append(Spacer(1, 20))

    # Process each type of ductwork
    for section in ("supply", "extract"):
        ...
        header_style = ParagraphStyle(
            ...
            fontSize=18,
            spaceAfter=10,
            textColor=colors.red if section == "supply" else colors.blue
        )
        story.append(Paragraph(f"{section.capitalize()} Ductwork Measurements", header_style))
        ...
        if lengths:
            ...
            # Create table style with color coding for the header
            header_color = colors.red if section == "supply" else colors.blue
